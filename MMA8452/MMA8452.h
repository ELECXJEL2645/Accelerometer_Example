...
private:
    ...
    //awakens device from standby mode
    void deviceWake();

    //sets the device sensitivity according to user specification
    void setRange(Range_setting const range);

    //reads the status of the PL_STATUS register
    char readPLStat();

    //sends a byte to a specificed register
    void sendByteToRegister(char byte,char reg);

    //read byte from specifide register
